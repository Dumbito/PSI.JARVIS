...
def test_gui_data_with_uninitialized_database_is_safe(tmp_path):
    ...
    assert service.project_papers("not-a-uuid") == ()
...
def test_main_window_builds_against_initialized_schema(tmp_path):
    ...
    with sqlite3.connect(database_path) as connection:
        initialize_schema(connection); connection.commit()
    app = QApplication.instance() or QApplication([])
...
def test_tutorial_supports_next_back_and_skip():
    ...
    dialog.next_button.click(); assert dialog.progress.text() == "Step 2 of 6"; assert dialog.back_button.isEnabled() is True
    dialog.back_button.click(); assert dialog.progress.text() == "Step 1 of 6"
    dialog.skip_button.click(); assert dialog.result() == TutorialDialog.Rejected
    dialog.deleteLater(); app.processEvents()


def test_gui_workflow_persists_project_scoped_corpus(tmp_path):
    database_path = tmp_path / "psi.db"
    csv_path = tmp_path / "papers.csv"
    csv_path.write_text("title,authors,abstract,doi,pmid,publication_year,journal\nNeural memory and cognition,Alice Smith;Bob Jones,Study of neural memory,,12345,2025,Journal of Cognition\n", encoding="utf-8")
    workflow = GuiWorkflowService(database_path)
    project = workflow.create_project(name="Memory review", research_question="How does neural memory support cognition?", topic="neural memory", inclusion=("cognition",), exclusion=())
    ...
    service = GuiDataService(database_path)
    assert service.snapshot().projects == 1
    assert service.snapshot().papers == 1
    ...
    papers = service.project_papers(str(project.project_id))
    assert len(papers) == 1
    assert papers[0].title == "Neural memory and cognition"
    assert papers[0].year == 2025
    assert papers[0].journal == "Journal of Cognition"
    assert papers[0].position == 0
