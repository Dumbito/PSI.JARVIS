...
class WebOfScienceJsonMapper:
    ...
    def map(self, response: RemoteAcquisitionResponse, receipt: AcquisitionReceipt) -> AcquisitionResult:
        try:
            ...
            return AcquisitionResult(receipt=receipt, issues=(AcquisitionIssue("invalid_format", f"Invalid Web of Science JSON: {exc}"),))
        hits = payload.get("hits", [])
        if not isinstance(hits, list):
            return AcquisitionResult(receipt=receipt, issues=(AcquisitionIssue("invalid_format", "Web of Science hits must be a list"),))
        papers: list[Paper] = []
        issues: list[AcquisitionIssue] = []
        for ordinal, entry in enumerate(hits, start=1):
            if not isinstance(entry, dict):
                issues.append(AcquisitionIssue("invalid_record", "Web of Science hit must be an object", "warning", ordinal))
                continue
    ...
    def _map_entry(self, receipt: AcquisitionReceipt, ordinal: int, entry: dict):
        ...
        authors = tuple(
            self._text(author.get("displayName") if isinstance(author, dict) else author)
            for author in authors_raw if self._text(author.get("displayName") if isinstance(author, dict) else author)
        )
        ...
        issues: list[AcquisitionIssue] = []
        if not title:
            issues.append(AcquisitionIssue("missing_title", "Web of Science record has no title", "warning", ordinal))
        if not source_record_id:
            issues.append(AcquisitionIssue("missing_wos_id", "Web of Science record has no UID", "warning", ordinal))
        ...
        return Paper(id=paper_id, title=title or "", authors=authors, doi=doi, publication_year=year, journal=journal, provenances=(provenance,)), tuple(issues)
...
class WebOfScienceAdapter(RemoteBibliographicAdapter):
    ...
    def __init__(self, fetcher: Callable[[BibliographicQuery], RemoteAcquisitionResponse], clock=None, mapper: WebOfScienceJsonMapper | None = None) -> None:
...
def build_wos_adapter(config: WebOfScienceTransportConfig, clock=None, opener: Callable[..., object] | None = None) -> WebOfScienceAdapter:
