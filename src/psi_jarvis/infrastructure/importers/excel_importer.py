...
class ExcelImporter:
    ...
    def acquire(self, request: AcquisitionRequest) -> AcquisitionResult:
        ...
        if not file_path.exists():
            return AcquisitionResult(receipt=None, issues=(AcquisitionIssue("source_unavailable", f"Excel file not found: {file_path}"),))
        if file_path.suffix.lower() not in {".xlsx", ".xls"}:
            return AcquisitionResult(receipt=None, issues=(AcquisitionIssue("invalid_format", "Expected an Excel file"),))
