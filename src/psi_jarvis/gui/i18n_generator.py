...
def install_translation_generator(window: QWidget, manager) -> QPushButton:
    button = QPushButton("Translation catalogs", window)
    button.setToolTip("Generate or update complete GUI translations with a local Ollama model.")
    button.clicked.connect(lambda: TranslationDialog(manager, window).exec())
    sidebar = window.findChild(QWidget, "sidebar")
    if sidebar is not None and sidebar.layout() is not None:
        sidebar.layout().insertWidget(max(0, sidebar.layout().count() - 1), button)
    return button
