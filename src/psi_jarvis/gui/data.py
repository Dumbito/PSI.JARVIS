...
from psi_jarvis.infrastructure.sqlite_metadata_history_repository import (
    SQLiteMetadataHistoryRepository,
)
...
class GuiDataService:
    ...
    def snapshot(self) -> DashboardSnapshot:
        ...
        with self._connect() as connection:
            try:
                for row in connection.execute(
                    "SELECT included, COUNT(*) AS count FROM screening_results GROUP BY included"
                ):
                    if row["included"]:
            ...
            try:
                for row in connection.execute(
                    "SELECT source_key, COUNT(*) AS count FROM acquisition_batches GROUP BY source_key"
                ):
        ...
        return DashboardSnapshot(
            ...
            papers=len(papers),
            screened=screened,
            pending=max(0, len(papers) - screened),
            included=included,
            excluded=excluded,
            conflicts=0,
    ...
    def sources(self) -> tuple[SourceSnapshot, ...]:
        ...
        try:
            ...
            manager = ScopusConnectionManager(
                environment.oauth,
                default_scopus_token_store(),
                oauth_client=ScopusOAuthClient(environment.oauth)
            ...
            return (
                SourceSnapshot(
                    ...
                    "PubMed",
                    "available",
                    "Public source without authentication.",
                ),
                SourceSnapshot(
                    "scopus",
                    "Scopus",
                    "unavailable",
                    f"Scopus configuration could not be inspected: {exc}",
                ),
                SourceSnapshot(
                    "web_of_science",
                    "Web of Science",
                    "unavailable",
                    "Connector available, but no credential is configured.",
                ),
                SourceSnapshot(
                    "zotero",
                    "Zotero",
        ...
        try:
            with self._connect() as connection:
                rows = connection.execute(
                    "SELECT sr.paper_id, p.title, p.publication_year, sr.included, sr.reason, sr.run_id, sr.criteria_version FROM screening_results sr JOIN papers p ON p.paper_id = sr.paper_id ORDER BY p.title COLLATE NOCASE"
                ).fetchall()
        except sqlite3.OperationalError:
            return ()
        return tuple(
    ...
    def screening_runs(self, limit: int = 100) -> tuple[ScreeningRunSnapshot, ...]:
        ...
        try:
            with self._connect() as connection:
                rows = connection.execute(
                    "SELECT run_id, project_id, criteria_version, started_at, total_input, unique_papers, duplicates_removed, screened_papers FROM screening_runs ORDER BY started_at DESC LIMIT ?",
                    (max(1, int(limit)),),
                ).fetchall()
    ...
    def metadata_quality(self) -> MetadataQualitySnapshot:
        ...
        return MetadataQualitySnapshot(
            total,
            sum(bool(p.abstract and p.abstract.strip()) for p in papers),
            sum(bool(p.authors) for p in papers),
            sum(bool(p.doi and p.doi.strip()) for p in papers),
            sum(bool(p.pmid and p.pmid.strip()) for p in papers),
            sum(bool(p.journal and p.journal.strip()) for p in papers),
            sum(p.publication_year is not None for p in papers),
        )
    ...
    def paper_provenance(self, paper_id: str) -> tuple[ProjectProvenanceSnapshot, ...]:
        ...
        try:
            with self._connect() as connection:
                rows = connection.execute(
                    "SELECT pp.paper_id,p.title,pp.batch_id,ab.source_key,pp.source_record_id,pp.record_ordinal,pp.format_name,pp.format_version,pp.mapping_version,pp.raw_record_sha256 FROM paper_provenances pp JOIN papers p ON p.paper_id=pp.paper_id JOIN acquisition_batches ab ON ab.batch_id=pp.batch_id WHERE pp.paper_id=? ORDER BY pp.record_ordinal",
                    (paper_id,),
                ).fetchall()
        except (sqlite3.OperationalError, ValueError):
            return ()
    ...
    def deduplication_analysis(self) -> DeduplicationAnalysis:
        ...
        return DeduplicationAnalysis(
            total_input=total_input, unique_papers=unique, duplicate_papers=dup
        )
    ...
    def project_snapshot(self, project_id: str) -> ProjectSnapshot | None:
        ...
        runs = tuple(
            run
            for run in self.screening_runs(limit=100000)
            if run.project_id == str(project.project_id)
        )
    ...
    def project_papers(self, project_id: str) -> tuple[ProjectPaperSnapshot, ...]:
        ...
        try:
            with self._connect() as connection:
                rows = connection.execute(
                    "SELECT p.paper_id,p.title,p.publication_year,p.journal,p.doi,p.pmid,cp.position FROM corpora c JOIN corpus_papers cp ON cp.corpus_id=c.corpus_id JOIN papers p ON p.paper_id=cp.paper_id WHERE c.project_id=? ORDER BY cp.position",
                    (project_id,),
                ).fetchall()
        except (sqlite3.OperationalError, ValueError):
            return ()
        return tuple(
            ProjectPaperSnapshot(
                row["paper_id"],
                row["title"],
                row["publication_year"],
                row["journal"],
    ...
    def project_screening_rows(self, project_id: str) -> tuple[ScreeningRow, ...]:
        ...
        try:
            with self._connect() as connection:
                rows = connection.execute(
                    "SELECT sr.paper_id,p.title,p.publication_year,sr.included,sr.reason,sr.run_id,sr.criteria_version FROM screening_results sr JOIN papers p ON p.paper_id=sr.paper_id JOIN screening_runs run ON run.run_id=sr.run_id WHERE run.project_id=? ORDER BY p.title COLLATE NOCASE",
                    (project_id,),
                ).fetchall()
        except (sqlite3.OperationalError, ValueError):
            return ()
    ...
    def project_provenance(
        self, project_id: str
    ) -> tuple[ProjectProvenanceSnapshot, ...]:
        if not self.database_path.exists():
            return ()
        try:
            with self._connect() as connection:
                rows = connection.execute(
                    "SELECT pp.paper_id,p.title,pp.batch_id,ab.source_key,pp.source_record_id,pp.record_ordinal,pp.format_name,pp.format_version,pp.mapping_version,pp.raw_record_sha256 FROM corpora c JOIN corpus_papers cp ON cp.corpus_id=c.corpus_id JOIN paper_provenances pp ON pp.paper_id=cp.paper_id JOIN papers p ON p.paper_id=pp.paper_id JOIN acquisition_batches ab ON ab.batch_id=pp.batch_id WHERE c.project_id=? ORDER BY cp.position,pp.record_ordinal",
                    (project_id,),
                ).fetchall()
        except (sqlite3.OperationalError, ValueError):
            return ()
    ...
    def screening_detail(
            ...
            with self._connect() as connection:
                if run_id:
                    row = connection.execute(
                        "SELECT sr.paper_id,p.title,sr.included,sr.reason,sr.run_id,sr.criteria_version,sa.audit_key FROM screening_results sr JOIN papers p ON p.paper_id=sr.paper_id LEFT JOIN screening_audits sa ON sa.paper_id=sr.paper_id AND sa.run_id=sr.run_id WHERE sr.paper_id=? AND sr.run_id=? LIMIT 1",
                        (paper_id, run_id),
                    ).fetchone()
                else:
                    row = connection.execute(
                        "SELECT sr.paper_id,p.title,sr.included,sr.reason,sr.run_id,sr.criteria_version,sa.audit_key FROM screening_results sr JOIN papers p ON p.paper_id=sr.paper_id LEFT JOIN screening_audits sa ON sa.paper_id=sr.paper_id AND sa.run_id=sr.run_id WHERE sr.paper_id=? ORDER BY rowid DESC LIMIT 1",
                        (paper_id,),
                    ).fetchone()
            if row is None:
                return None
        ...
        return ScreeningDetail(
            ...
            row["run_id"],
            row["criteria_version"],
            tuple(audit.matched_rules) if audit else (),
            tuple(audit.failed_rules) if audit else (),
            str(row["audit_key"]) if row["audit_key"] else None,
        )
