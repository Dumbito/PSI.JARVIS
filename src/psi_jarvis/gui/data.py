...
class GuiDataService:
    ...
    def screening_detail(
            ...
            with self._connect() as connection:
                if run_id:
                    row = connection.execute(
                        "SELECT sr.paper_id,p.title,sr.included,sr.reason,sr.run_id,sr.criteria_version FROM screening_results sr JOIN papers p ON p.paper_id=sr.paper_id WHERE sr.paper_id=? AND sr.run_id=? LIMIT 1",
                        (paper_id, run_id),
                    ).fetchone()
                else:
                    row = connection.execute(
                        "SELECT sr.paper_id,p.title,sr.included,sr.reason,sr.run_id,sr.criteria_version FROM screening_results sr JOIN papers p ON p.paper_id=sr.paper_id WHERE sr.paper_id=? ORDER BY rowid DESC LIMIT 1",
                        (paper_id,),
                    ).fetchone()
            if row is None:
                return None
        ...
        return ScreeningDetail(
            ...
            f"{row['run_id']}:{row['paper_id']}" if audit else None,
        )
    ...
    def sources(self) -> tuple[SourceSnapshot, ...]:
        try:
            ...
            return (
                SourceSnapshot("unknown", "Bibliographic sources", "error", str(exc)),
            )
