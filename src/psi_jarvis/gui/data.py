...
from psi_jarvis.infrastructure.auth import ScopusConnectionManager, ScopusOAuthClient, default_scopus_token_store, load_scopus_environment
...
class SourceSnapshot:
    key: str; display_name: str; status: str; detail: str = ""
@dataclass(frozen=True)
class ScreeningRow:
    paper_id: str; title: str; year: int | None; decision: str; reason: str; run_id: str | None; criteria_version: str | None
@dataclass(frozen=True)
class AuditRow:
    changed_at: str; paper_id: str; source_key: str; source_record_id: str | None; changed_fields: tuple[str, ...]
@dataclass(frozen=True)
class ScreeningRunSnapshot:
    run_id: str; project_id: str | None; criteria_version: str; started_at: str; total_input: int; unique_papers: int; duplicates_removed: int; screened_papers: int
@dataclass(frozen=True)
class MetadataQualitySnapshot:
    total_papers: int = 0; with_abstract: int = 0; with_authors: int = 0; with_doi: int = 0; with_pmid: int = 0; with_journal: int = 0; with_year: int = 0
@dataclass(frozen=True)
class ProjectSnapshot:
    project_id: str; name: str; research_question: str; topic: str; inclusion: tuple[str, ...]; exclusion: tuple[str, ...]; created_at: str; screening_runs: int; screened_papers: int
@dataclass(frozen=True)
class ProjectPaperSnapshot:
    paper_id: str; title: str; year: int | None; journal: str | None; doi: str | None; pmid: str | None; position: int
@dataclass(frozen=True)
class ProjectProvenanceSnapshot:
    paper_id: str; title: str; batch_id: str; source_key: str; source_record_id: str | None; record_ordinal: int; format_name: str; format_version: str; mapping_version: str; raw_record_sha256: str
@dataclass(frozen=True)
class ScreeningDetail:
    paper_id: str; title: str; decision: str; reason: str; run_id: str | None; criteria_version: str | None; matched_rules: tuple[str, ...] = (); failed_rules: tuple[str, ...] = (); audit_id: str | None = None
...
class GuiDataService:
    ...
    def snapshot(self) -> DashboardSnapshot:
        if not self.database_path.exists(): return DashboardSnapshot()
        try: projects = SQLiteProjectRepository(self.database_path).list_all(); papers = SQLitePaperRepository(self.database_path).list_all()
        except sqlite3.OperationalError: return DashboardSnapshot()
        included = excluded = 0; source_counts: dict[str, int] = {}
        with self._connect() as connection:
            try:
                for row in connection.execute("SELECT included, COUNT(*) AS count FROM screening_results GROUP BY included"):
                    if row["included"]: included = int(row["count"])
                    else: excluded = int(row["count"])
            except sqlite3.OperationalError: pass
            try:
                for row in connection.execute("SELECT source_key, COUNT(*) AS count FROM acquisition_batches GROUP BY source_key"): source_counts[str(row["source_key"])] = int(row["count"])
            except sqlite3.OperationalError: pass
        screened = included + excluded
        return DashboardSnapshot(projects=len(projects), papers=len(papers), screened=screened, pending=max(0, len(papers)-screened), included=included, excluded=excluded, conflicts=0, source_counts=tuple(sorted(source_counts.items())))
    def projects(self):
        if not self.database_path.exists(): return ()
        try: return SQLiteProjectRepository(self.database_path).list_all()
        except sqlite3.OperationalError: return ()
    def papers(self):
        if not self.database_path.exists(): return ()
        try: return SQLitePaperRepository(self.database_path).list_all()
        except sqlite3.OperationalError: return ()
    ...
    def metadata_quality(self) -> MetadataQualitySnapshot:
        papers = self.papers(); total = len(papers)
    ...
    def audit_rows(self, limit: int = 200) -> tuple[AuditRow, ...]:
        if not self.database_path.exists(): return ()
        try: changes = SQLiteMetadataHistoryRepository(self.database_path).list_all()
        except (sqlite3.OperationalError, ValueError): return ()
        return tuple(AuditRow(change.changed_at.isoformat(), str(change.paper_id), change.source_key, change.source_record_id, change.changed_fields) for change in changes[-limit:])
    ...
    def rule_analysis(self) -> RuleAnalysis: return RuleAnalysis.from_audits(self.all_audits())
    def exclusion_reason_analysis(self) -> ExclusionReasonAnalysis: return ExclusionReasonAnalysis.from_audits(self.all_audits())
    def author_analysis(self) -> AuthorAnalysis: return AuthorAnalysis.from_papers(self.papers())
    def journal_analysis(self) -> JournalAnalysis: return JournalAnalysis.from_papers(self.papers())
    def publication_year_analysis(self) -> PublicationYearAnalysis: return PublicationYearAnalysis.from_papers(self.papers())
    def deduplication_analysis(self) -> DeduplicationAnalysis:
        runs=self.screening_runs(limit=100000); total_input=sum(r.total_input for r in runs); unique=sum(r.unique_papers for r in runs); dup=sum(r.duplicates_removed for r in runs)
        return DeduplicationAnalysis(total_input=total_input, unique_papers=unique, duplicate_papers=dup)
    def project(self, project_id: str):
        if not self.database_path.exists(): return None
        try: return SQLiteProjectRepository(self.database_path).get(UUID(project_id))
        except (sqlite3.OperationalError, ValueError): return None
    ...
    def screening_detail(self, paper_id: str, run_id: str | None = None) -> ScreeningDetail | None:
        ...
        try:
            with self._connect() as connection:
                if run_id: row=connection.execute("SELECT sr.paper_id,p.title,sr.included,sr.reason,sr.run_id,sr.criteria_version FROM screening_results sr JOIN papers p ON p.paper_id=sr.paper_id WHERE sr.paper_id=? AND sr.run_id=? LIMIT 1",(paper_id,run_id)).fetchone()
                else: row=connection.execute("SELECT sr.paper_id,p.title,sr.included,sr.reason,sr.run_id,sr.criteria_version FROM screening_results sr JOIN papers p ON p.paper_id=sr.paper_id WHERE sr.paper_id=? ORDER BY rowid DESC LIMIT 1",(paper_id,)).fetchone()
            if row is None: return None
            audit=SQLiteScreeningAuditRepository(self.database_path).get(UUID(paper_id),UUID(row["run_id"]) if row["run_id"] else None)
        except (sqlite3.OperationalError, ValueError): return None
        return ScreeningDetail(row["paper_id"],row["title"],"Included" if row["included"] else "Excluded",row["reason"],row["run_id"],row["criteria_version"],tuple(audit.matched_rules) if audit else (),tuple(audit.failed_rules) if audit else (),f"{row['run_id']}:{row['paper_id']}" if audit else None)
    def sources(self) -> tuple[SourceSnapshot, ...]:
        try:
            config=load_scopus_environment(); manager=ScopusConnectionManager(config.oauth,default_scopus_token_store(),oauth_client=ScopusOAuthClient(config.oauth) if config.oauth is not None else None,transport_config=config.transport); registry=build_default_source_connection_registry(manager)
            return tuple(SourceSnapshot(state.source.key,state.source.display_name,state.status.value,state.detail) for state in registry.inspect_all())
        except Exception as exc: return (SourceSnapshot("unknown","Bibliographic sources","error",str(exc)),)
