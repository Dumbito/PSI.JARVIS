...
class ProjectPaperDialog(QDialog):
    ...
    def __init__(
        ...
        project_id: str,
        paper_id: str,
        open_screening: Callable[[str], None] | None = None,
    ...
    def _screening(self) -> QWidget:
        ...
        rows = tuple(
            row
            for row in self.data.project_screening_rows(self.project_id)
            if row.paper_id == self.paper_id
        )
        table = self._table(
        ...
        layout.addWidget(
            QLabel(
                f"Persisted screening decisions · {len(rows):,} result(s) · Double-click a run to navigate"
            )
        )
    ...
    def _provenance(self) -> QWidget:
        ...
        rows = tuple(
            row
            for row in self.data.project_provenance(self.project_id)
            if row.paper_id == self.paper_id
        )
        table = self._table(
            [
            ...
            [
                (
                    ...
                    row.batch_id,
                    row.record_ordinal,
                    f"{row.format_name} {row.format_version}",
                    row.mapping_version,
    ...
    def _evidence(self) -> QWidget:
        ...
        if not rows:
            layout.addWidget(
                QLabel(
                    "No persisted screening evidence is available for this paper in this project."
                )
            )
            layout.addStretch()
            return page
        for row in rows:
            ...
            block_layout.addWidget(
                QLabel(
                    f"Run: {detail.run_id or '—'} · Criteria: {detail.criteria_version or '—'}"
                )
            )
            ...
            block_layout.addWidget(
                QLabel(
                    "Matched rules: "
                    + (", ".join(detail.matched_rules) if detail.matched_rules else "—")
                )
            )
            block_layout.addWidget(
                QLabel(
                    "Failed rules: "
                    + (", ".join(detail.failed_rules) if detail.failed_rules else "—")
                )
            )
...
class ScreeningRunDialog(QDialog):
    ...
    def __init__(
        ...
        project_id: str,
        run_id: str,
        open_paper: Callable[[str], None] | None = None,
        open_screening: Callable[[str], None] | None = None,
        parent: QWidget | None = None,
    ) -> None:
        super().__init__(parent)
        ...
        self.project_id = project_id
        self.run_id = run_id
        self.open_paper = open_paper
        self.open_screening = open_screening
        self.setWindowTitle("Screening run")
    ...
    def _build(self) -> None:
        ...
        run = next(
            (
                x
                for x in self.data.screening_runs(limit=100000)
                if x.run_id == self.run_id
            ),
            None,
        )
        if run is None:
            root.addWidget(QLabel("This screening run is no longer available."))
            self._close(root)
            return
        title = QLabel(f"Screening run · {run.run_id}")
        title.setObjectName("dialogTitle")
        title.setWordWrap(True)
        root.addWidget(title)
        summary = QGridLayout()
        values = (
            ...
            ("Duplicates removed", run.duplicates_removed),
            ("Screened", run.screened_papers),
        )
        for row, (label, value) in enumerate(values):
            summary.addWidget(QLabel(f"{label}:"), row // 2 * 1, row % 2 * 2)
            summary.addWidget(QLabel(str(value)), row // 2, row % 2 * 2 + 1)
        root.addLayout(summary)
        rows = tuple(
            x
            for x in self.data.project_screening_rows(self.project_id)
            if x.run_id == self.run_id
        )
        ...
        for r, item in enumerate(rows):
            for c, value in enumerate(
                (
                    ...
                    item.decision,
                    item.reason,
                    item.criteria_version or "—",
                )
            ):
                table.setItem(r, c, QTableWidgetItem(str(value)))
            table.item(r, 0).setData(Qt.UserRole, item.paper_id)
        table.doubleClicked.connect(lambda: self._open_selected(table))
        root.addWidget(
            QLabel(
                f"Persisted results in this run · {len(rows):,} result(s) · Double-click a paper to inspect it"
            )
        )
        ...
        if self.open_screening is not None:
            ...
            button = QPushButton("Open in Screening")
            button.clicked.connect(lambda: self.open_screening(self.run_id))
    ...
    def _open_selected(self, table: QTableWidget) -> None:
        if self.open_paper is None:
            return
        row = table.currentRow()
        if row >= 0:
            paper_id = table.item(row, 0).data(Qt.UserRole)
            if paper_id:
                self.open_paper(str(paper_id))
...
class ProjectWorkspaceView(QWidget):
    ...
    def _build(self) -> None:
        ...
        note = QLabel(
            "Read-only project context. Scientific decisions remain owned by the deterministic screening pipeline."
        )
    ...
    def _overview(self, snapshot: ProjectSnapshot) -> QWidget:
        ...
        grid = QGridLayout()
        metrics = (
            ("Screening runs", snapshot.screening_runs),
            ("Screened papers", snapshot.screened_papers),
            ("Corpus papers", len(self.data.project_papers(self.project_id))),
            ("Provenance records", len(self.data.project_provenance(self.project_id))),
        )
        ...
        layout.addWidget(
            QLabel(f"Research question: {snapshot.research_question or '—'}")
        )
        layout.addWidget(
            QLabel(
                "Inclusion: "
                + ("; ".join(snapshot.inclusion) if snapshot.inclusion else "—")
            )
        )
        layout.addWidget(
            QLabel(
                "Exclusion: "
                + ("; ".join(snapshot.exclusion) if snapshot.exclusion else "—")
            )
        )
        layout.addStretch()
        return page
    ...
    def _papers(self) -> QWidget:
        ...
        search.setPlaceholderText(
            "Search this project's papers by title, journal, DOI, or PMID…"
        )
        table = self._table(
        ...
        layout.addWidget(
            QLabel(
                f"Canonical corpus · {len(papers):,} papers · Double-click a paper to inspect it"
            )
        )
        layout.addWidget(table, 1)
        return page
    ...
    def _open_paper_for_row(self, table: QTableWidget) -> None:
        ...
        if not paper_id:
            return
        self._paper_dialog = ProjectPaperDialog(
            self.data, self.project_id, str(paper_id), self.open_screening, self
        )
    ...
    def _screening(self) -> QWidget:
        ...
        for r, x in enumerate(rows):
            table.item(r, 0).setData(Qt.UserRole, (x.paper_id, x.run_id))
        table.doubleClicked.connect(lambda: self._open_screening_selection(table))
        search.textChanged.connect(lambda text: self._filter_table(table, text))
        layout.addWidget(search)
        layout.addWidget(
            QLabel(
                f"Persisted screening results · {len(rows):,} result(s) · Double-click a result to inspect its paper"
            )
        )
        layout.addWidget(table, 1)
        return page
    ...
    def _open_screening_selection(self, table: QTableWidget) -> None:
        ...
        self._paper_dialog.raise_()
        self._paper_dialog.activateWindow()
        if self.open_screening is not None and run_id:
            self.open_screening(str(run_id))
    ...
    def _runs(self) -> QWidget:
        ...
        runs = tuple(
            run
            for run in self.data.screening_runs(limit=100000)
        ...
        layout.addWidget(
            QLabel(
                f"Screening run history · {len(runs):,} run(s) · Double-click to inspect run"
            )
        )
        layout.addWidget(table, 1)
        return page
    ...
    def _open_run_from_table(self, table: QTableWidget, runs) -> None:
        ...
        if row < 0:
            return
        run_id = str(runs[row].run_id)
        self._run_dialog = ScreeningRunDialog(
            self.data,
            self.project_id,
            run_id,
            self._open_paper,
    ...
    def _open_paper(self, paper_id: str) -> None:
        self._paper_dialog = ProjectPaperDialog(
            self.data, self.project_id, paper_id, self.open_screening, self
        )
        self._paper_dialog.show()
        self._paper_dialog.raise_()
        self._paper_dialog.activateWindow()
        ...
        table = self._table(
            ...
            [
                (
                    ...
                    x.batch_id,
                    x.record_ordinal,
                    f"{x.format_name} {x.format_version}",
                    x.mapping_version,
                    x.raw_record_sha256,
                )
                for x in rows
